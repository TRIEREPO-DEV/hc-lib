angular.module('hcLib').service('salesbillService', ['httpService', function(httpService) {
    ...
    this.downloadScheduleSalesExcel = downloadScheduleSalesExcel;
    ...
    function downloadScheduleSalesExcel(params){
        var filename = 'Saleswise_';
        if(params.schedule){
            filename += params.schedule;
        }
        if(params.toDate){
            filename += params.toDate;
        }
        if(params.fromDate){
            filename += params.fromDate;
        }
        filename += '.xlsx';
        return httpService.download('salesbill/salesmedicinesbyschedule/download',null,params,filename);
    }
}]);
